...
int find_berth(int zhen,int id)
{
    ...
    {
        if (15000 - zhen - boat[id].num / berth[t].loading_speed - 2 * berth[t].transport_time < 0)
        {
            continue;
        }
        if (vis_berth[t] == 0)
...
int find_berth_2(int zhen,int id)
{
    ...
    for (int t = 0; t < berth_num; t++)
    {
        if (15000 - zhen - boat[id].num / berth[t].loading_speed - berth[t].transport_time - 500 < 0)
        {
            continue;
        }
        if (vis_berth[t] == 0)
...
int Input(int zhen)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        {
            // 如果时间不够
            if (14970 - berth[boat[i].berth].transport_time - zhen < 0)
            {
                printf("go %d\n", i);
...
void bfs_good(int x, int y, int zhen, int id)
{
    ...
    while (!q.empty())
    {
        ...
        {
            double w = good_value[now.first][now.second] / 200.0 * w_good_val - dis / 400.0 * w_good_dis;
            if (w > mx)
            {
                mx = w;
                pos = now;
