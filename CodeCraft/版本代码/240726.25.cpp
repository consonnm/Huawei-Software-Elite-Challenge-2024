...
// 30
double w_good_val = 0.2688268268151062, w_good_dis = 0.6911235572793718;
double w_boat_speed = 0.20896958911853852, w_boat_size = 1.7464252561010907, w_boat_transport = 0.5328674387374163;
double w_berth_fill = 1.8985547540451901, w_berth_dis = 0.8384535517011492, w_berth_vis = 1.4829196534346907;
double w_near_dis = 0.04425803665773132, w_near_val = 0.8142301472747412;
double max_near_dis = 100;


         
// double w_good_val = 0.9945136435835381, w_good_dis = 0.7704898963889539;
// double w_boat_speed = 0.25610043304165847, w_boat_size = 1.062010716501154, w_boat_transport = 0.03255963479112068;
// double w_berth_fill = 0.2686989925778836, w_berth_dis = 0.0023343942588910376, w_berth_vis =1.6644247432907213;
// double w_near_val = 0.04412405903968931, w_near_dis = 0.007085888346385573;
// double max_near_dis = 100;
...
void bfs_good(int x, int y, int zhen, int id)
{
    ...
    while (fa[temp_x][temp_y][0] != x || fa[temp_x][temp_y][1] != y)
    {
        cycle++;
        if (cycle > 10000){
            // printf("move %d %d\n", 10000000+x, 1000000+y);
            // printf("move %d %d\n", 10000000+pos.first, 1000000+pos.second);
            // printf("move %d %d\n", 100+id,100000);
            // break;
            exit(0);
        }
    ...
    for (int i = 0; i < berth_num; i++)
    {
        ...
        if (zhen >= out)
        {
            ...
            if (berth[i].size > boat[(vis_berth[i] - 1)].num)
                continue;
            search_berth[++search_size].x = pre_dis_berth[i][x][y] * w_berth_dis - berth[i].near_val / (mx_near_val + 1) * w_near_val;
            search_berth[search_size].id = i;
        }
        else
        {
            search_berth[++search_size].x = -(berth[i].size < boat_capacity) * 100000.0 +
                                            pre_dis_berth[i][x][y] / mx_pre_dis * w_berth_dis - berth[i].near_val / (mx_near_val + 1) * w_near_val;
            search_berth[search_size].id = i;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    for (int zhen = 1; zhen <= 15000; zhen++)
    {
        ...
        // if (zhen >= 14995)
        // {
        //     logFile << "total_num: " << total_num << " disapper_num: " << disapper_num << " get_num: " << get_num << " get_num_val: " << get_num_val << endl;
        // }
        // if (zhen >= 14995)
        // {
        //     for (int i = 0; i < berth_num; i++)
        ...
        //     for (int i = 0; i < berth_num; i++)
        //     {
        //         logFile << i << " " << berth[i].x << " " << berth[i].y << " " << berth[i].transport_time << " " << berth[i].loading_speed << " " << berth[i].size << " " << berth[i].load << " " << berth[i].arrive << " " << berth[i].near_val << endl;
        //     }
        // }
        for (int i = 1; i <= n; i++)
        {
            for (int j = 1; j <= n; j++)
            {
                ...
                {
                    if (pre_dis_berth[k][i][j] <= max_near_dis && zhen - good_time[i][j] == 1000 && good_time[i][j] > 0)
                    {
                        berth[k].near_val--;
                    }
                }
